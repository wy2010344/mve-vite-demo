...
function forall(atp, f) {
  const [atpt, atpk] = untyped(atp);
  assert(atpk == 'Type' || atpk == 'Kind', `illegal arg type/kind: ${atpk}`);
  const [res, tpe] = untyped(f(typed('x?', atpt))); //返回类型
  return typed(
 ...
 */
const polyId = fun(Type, t => fun(t, x => x));
// let polyId = fun(Type, t => fun(t, x => fun(t, y => y)(x)))
printt('polyId', polyId);

//皮来诺数类型
const N = constant('N', Type);
const z = constant('z', N);
const s = constant(
  's',
  forall(N, x => N)
);
...
//邱奇数
const ff = T => forall(T, x => T); // fun type T->T
const ChurchT = forall(Type, N => forall(N, z => forall(ff(N), s => N)));
const ChurchN = f => fun(Type, N => fun(N, z => fun(ff(N), s => f(N)(z)(s))));
//邱奇数转化成皮来诺数
const peano = fun(ChurchT, n => n(N)(z)(s));
const three = ChurchN(N => z => s => s(s(s(z))));
