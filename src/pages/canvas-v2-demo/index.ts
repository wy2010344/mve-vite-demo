...
import {
  ...
  LayoutNode,
  engineGlobalContext,
} from 'mve-dom-helper/canvasEngine';
...
export default function () {
  ...
  canvasEl.width = 600;
  canvasEl.height = 500;
  canvasEl.className = 'touch-none';
  canvasEl.style.cssText =
    'display:block;margin:20px auto;border:1px solid #ddd;border-radius:8px;';
  ...
  const renderer = new Renderer({
    ...
    buildChildren() {
      ...
      const title = new TextNode(h);
      title.text = 'Canvas Engine v2';
      title.fontSize = 24;
      title.fontWeight = 700;
      title.color = rgba(30, 30, 30);

      const desc = new WrappedTextNode(h);
      desc.text = '点击色块切换颜色 · 拖拽粉色方块 · 滚轮滚动列表';
      desc.fontSize = 13;
      desc.color = rgba(120, 120, 120);
  ...
  canvasEl.addEventListener('mousedown', e => {
    ...
    renderer.dispatchMouseDown(e.clientX - r.left, e.clientY - r.top);
  });
  canvasEl.addEventListener('mouseup', e => {
    const r = canvasEl.getBoundingClientRect();
    renderer.dispatchMouseUp(e.clientX - r.left, e.clientY - r.top);
  });
  canvasEl.addEventListener('mousemove', e => {
    const r = canvasEl.getBoundingClientRect();
    renderer.dispatchMouseMove(e.clientX - r.left, e.clientY - r.top);
  });
  canvasEl.addEventListener(
    'wheel',
    e => {
      const r = canvasEl.getBoundingClientRect();
      renderer.dispatchMouseWheel(
        e.clientX - r.left,
        e.clientY - r.top,
        e.deltaY
      );
...
class ScrollSection extends RectNode {
  ...
  override draw(ctx: CanvasRenderingContext2D) {
    ...
    for (let i = 0; i < this.itemCount; i++) {
      ...
      ctx.fillRect(1, y, w - 2, this.itemH);
      ctx.fillStyle = '#444';
      ctx.font = '13px sans-serif';
      ctx.textBaseline = 'middle';
