...
import { addEffect, arrayCountCreateWith, bigSpinDeviceToEnd, bigSpinStepTo, createBigSpin, createSignal, EmptyFun, memo } from "wy-helper";
...
export default function () {
  ...
  const prizes = memo(
    () => arrayCountCreateWith(prizeCount.get(),
      i => {
        return {
          id: i,
  ...
  let cancel: EmptyFun
  hookTrackSignal(drawValue, function (onDraw) {
    if (onDraw) {
      const call = onDrawEffect()
      cancel = subscribeRequestAnimationFrame(function (time) {
        call(time, cancel)
      })
    } else {
      cancel?.()
  ...
  fdom.div({
    ...
    children() {
      ...
      fdom.div({
        className: 'absolute w-[525px] h-[525px] left-[50%] translate-x-[-50%] top-[70px]',
        children() {
          fdom.div({
...
function toConicColor<T>(newList: NewRow<T>[], getColor: (n: T) => string) {
  ...
  newList.forEach((row) => {
    list.push(
      `${getColor(row.data)} ${row.beforePercent * CI}deg ${(row.beforePercent + row.percent) * CI
      }deg`
    );
  });
