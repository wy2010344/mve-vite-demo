...
import {
  ...
  type QualityMetrics,
  type RiverIndicator,
} from './justification-comparison.model';
...
export function syncCssRiverOverlay(
  ...
  for (
    ...
    paragraphIndex < dom.cssParagraphs.length;
    paragraphIndex++
  ) {
      ...
      for (
        ...
        if (segment.kind === 'space') {
          if (showIndicators && line.spacing.isRiver) {
            const indicator = getRiverIndicator(
              line.spacing.width,
              normalSpaceWidth
            );
            if (indicator !== null) {
