...
import {
  ...
  PREFIXES,
  SUFFIXES,
} from './justification-comparison.data';
...
export function createDemoResources(): DemoResources {
  ...
  return {
    basePreparedParagraphs: PARAGRAPHS.map(paragraph =>
      prepareWithSegments(paragraph, FONT)
    ),
    hyphenatedPreparedParagraphs: PARAGRAPHS.map(paragraph =>
      prepareWithSegments(hyphenateParagraphText(paragraph), FONT)
    ),
...
export function buildDemoFrame(
  ...
  const cssParagraphs = layoutParagraphsGreedy(
    resources.basePreparedParagraphs,
    innerWidth,
    resources.hyphenWidth
  );
  const hyphenParagraphs = layoutParagraphsGreedy(
    resources.hyphenatedPreparedParagraphs,
    innerWidth,
    resources.hyphenWidth
  );
  const optimalParagraphs = layoutParagraphsOptimal(
    resources.hyphenatedPreparedParagraphs,
    innerWidth,
    resources
  );
  ...
  return {
    ...
    hyphen: buildCanvasColumnFrame(
      controls.colWidth,
      hyphenParagraphs,
      resources.normalSpaceWidth
    ),
    optimal: buildCanvasColumnFrame(
      controls.colWidth,
      optimalParagraphs,
      resources.normalSpaceWidth
    ),
...
export function getRiverIndicator(
  ...
  const intensity = Math.min(
    1,
    (spaceWidth / normalSpaceWidth - RIVER_THRESHOLD) / RIVER_THRESHOLD
  );
  return {
...
function layoutParagraphsGreedy(
  ...
  for (let index = 0; index < preparedParagraphs.length; index++) {
    paragraphs.push(
      layoutParagraphGreedy(preparedParagraphs[index]!, maxWidth, hyphenWidth)
    );
  }
  return paragraphs;
}
...
function buildMeasuredLineFromLayoutResult(
  ...
  for (
    let segIndex = start.segmentIndex;
    segIndex < end.segmentIndex;
    segIndex++
  ) {
...
function layoutParagraphsOptimal(
  ...
  for (let index = 0; index < preparedParagraphs.length; index++) {
    paragraphs.push(
      layoutParagraphOptimal(preparedParagraphs[index]!, maxWidth, resources)
    );
  }
  return paragraphs;
}
...
function computeMetrics(
  ...
  for (
    ...
    for (let lineIndex = 0; lineIndex < paragraph.length; lineIndex++) {
      ...
      const deviation =
        Math.abs(metricSpaceWidth - normalSpaceWidth) / normalSpaceWidth;
