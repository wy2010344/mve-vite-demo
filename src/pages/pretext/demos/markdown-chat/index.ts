...
function createMessageShell(
  role: ChatMessageInstance['frame']['role']
): CachedRow {
...
export default function () {
  fdom.main({
    ...
    children() {
      ...
      fdom.div({
        ...
        children() {
          fdom.div({
            className: 'occlusion-banner occlusion-banner--top',
            id: 'top-banner',
            children() {
              fdom.button({
                className: 'virtualization-toggle',
        ...
        `,
          ...
          fdom.div({
            className: 'occlusion-banner occlusion-banner--bottom',
            id: 'bottom-banner',
          });
        },
      });
  ...
  addEffect(() => {
    ...
    domCache.viewport.addEventListener('scroll', scheduleRender, {
      passive: true,
    });
    ...
    function render(): void {
      ...
      const isCompactOcclusionChrome =
      ...
      domCache.root.style.setProperty(
        '--occlusion-banner-height',
        `${occlusionBannerHeight}px`
      );
      domCache.root.style.setProperty(
        '--occlusion-banner-padding-block',
        isCompactOcclusionChrome ? '6px' : '12px'
      );
      domCache.root.style.setProperty(
        '--virtualization-toggle-padding-block',
        isCompactOcclusionChrome ? '8px' : '10px'
      );
      domCache.root.style.setProperty(
        '--virtualization-toggle-padding-inline',
        isCompactOcclusionChrome ? '12px' : '14px'
      );
      domCache.root.style.setProperty(
        '--virtualization-toggle-font-size',
        isCompactOcclusionChrome ? '11px' : '12px'
      );
      domCache.shell.dataset['visualization'] = isVisualizationOn
        ? 'on'
        : 'off';
      ...
      domCache.toggleButton.textContent = isVisualizationOn
        ...
        : 'Show virtualization mask';
      domCache.toggleButton.setAttribute(
        'aria-pressed',
        String(isVisualizationOn)
      );
    ...
    function projectVisibleRows(
      ...
      for (
        let index = Math.max(previousStart, end);
        index < previousEnd;
        index++
      ) {
        const node = domCache.rows[index];
      ...
      if (overlapStart >= overlapEnd) {
        for (let index = start; index < end; index++) {
          ...
          projectMessageNode(
            cachedRow,
            frame.messages[index]!.frame,
            frame.messages[index]!.top
          );
        ...
        for (let index = overlapStart - 1; index >= start; index--) {
          ...
          if (anchorRow === null) {
            ...
            cachedRow.row.parentNode !== domCache.canvas ||
