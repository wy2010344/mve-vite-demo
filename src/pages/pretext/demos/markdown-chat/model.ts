...
import { BASE_MESSAGE_SPECS, type MarkdownChatSeed } from './data';
...
export function findVisibleRange(
  ...
  const maxY = Math.max(
    minY,
    scrollTop + viewportHeight - bottomOcclusionHeight
  );
  let low = 0;
  ...
  function walk(tokenList: readonly Token[], marks: MarkState): void {
    for (let index = 0; index < tokenList.length; index++) {
      ...
      switch (token.type) {
        ...
        case 'checkbox': {
          pushPiece(
            createTextPiece(token.checked ? '[x] ' : '[ ] ', marks, variant)
          );
          continue;
        }
