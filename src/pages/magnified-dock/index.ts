...
参考[https://buildui.com/recipes/magnified-dock](https://buildui.com/recipes/magnified-dock)
  ...
  dom
    .div({
      className:
        'mx-auto hidden h-16 items-end gap-3 px-2 pb-3 sm:flex relative',
      onMouseMove(e) {
    ...
    .render(() => {
      dom
        .div({
          className:
            'absolute rounded-2xl inset-y-0 bg-gray-700 border border-gray-600 -z-10',
          style: {
            left() {
              return `${left()}px`;
            },
      ...
      APPS.forEach(app => {
        ...
        const btn = dom
          .button({
            className:
              'aspect-square block w-10 rounded-full bg-white shadow origin-bottom relative group/abc',
            style: {
              // scale,
              transform() {
            ...
            async onClick() {
              async function fun() {
                const n = await y.changeTo(
                  -40,
                  tween((0.7 / 2) * 1000, cubicBezier(0, 0, 0.2, 1))
                );
                const x = await y.changeTo(
                  0,
                  tween((0.7 / 2) * 1000, cubicBezier(0.8, 0, 1, 1))
                );
                console.log('x', n, x);
              }
              await fun();
            ...
            dom
              ...
              .render(() => {
                dom
                  .div({
                    className:
                      'w-2 h-2 bg-gray-700 absolute -bottom-1 left-[50%] translate-x-[-50%] rotate-45',
                  })
                  .render();
                dom
                  .div({
