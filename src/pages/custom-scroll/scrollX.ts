...
import { AnimateSignal, AnimateSignalConfig, ClampingScrollFactory, createSignal, DeltaXSignalAnimationConfig, destinationWithMargin, eventGetPageX, eventGetPageY, getMaxScroll, GetValue, PointKey, scrollForEdge, ScrollFromPage, SetValue } from "wy-helper"
...
export class OnScroll {
  ...
  private pointerScroll() {
    const that = this
    this.config.container.addEventListener("pointerdown", function (e) {
      that.scroll.stop()
      pointerMove(ScrollFromPage.from(e, {
        getPage: that.config.direction == 'x' ? eventGetPageX : eventGetPageY,
        scrollDelta(delta, velocity) {
          that.drag(delta)
        },
        onFinish: that.destination
      }))
