...
import { createSignal, FrictionalFactory, EaseType, spring } from 'wy-helper';
import { NumberRange, renderNumberRange } from '../../renderRange';
import fixRightTop from '../../fixRightTop';
import themeDropdown from '../../themeDropdown';

import bezierCanvasView from './bezier-canvas-view';
import {
  easePoly,
  easeCirc,
  easeSine,
...
import renderClamping from './clamping-scroll';
  ...
  renderFullScreen(function ({ width, height }) {
    renderIf(
      ...
      function () {
        fdom.div({
          className:
            'w-full h-full overflow-y-auto flex flex-col items-center pt-1 pb-1 touch-none',
          children() {
            fdom.div({
...
function renderFrc() {
  ...
  drawUnknownEndView({
    getAnimationFun() {
      return {
        callback(height) {
          return FrictionalFactory.get(deceleration.value.get())
            .getFromDistance(height)
            .animationConfig();
        },
      };
    },
