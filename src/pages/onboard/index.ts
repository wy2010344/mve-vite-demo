import { dom, fdom, zdom, renderText, renderTextContent } from 'mve-dom';
...
export default function () {
  renderMobileView(function ({ width }) {
    ...
    hookDestroy(
      subscribeEventListener(window, 'pointerdown', e => {
        pointerMove(
      ...
      children() {
        ...
        fdom.div({
          ...
          s_transform() {
            ...
            return `perspective(300px) rotateY(${interpolateRotate(
              -scrollX.get()
            )}deg) scale(${interpolateScale(x)})`;
          },
          s_backgroundColor() {
            return rgbFromBgColor(
              interpolateAnimationBgColor(Math.abs(scrollX.get()))
            );
          },
        });
        ...
        fdom.button({
          className:
            'absolute z-1 top-0 w-[100px] h-[100px] flex items-center justify-center rounded-full bg-transparent',
          s_opacity() {
    ...
    fdom.div({
      ...
      children() {
        renderArray(cacheList, function (row, getIndex) {
          zdom.div({
            attrs(m) {
              m.s_color = row.textColor;
              m.className =
                'h-full flex flex-col items-center perspective-near';
              const i = getIndex();
              if (i != 1) {
                m.s_position = 'absolute';
                m.s_inset = 0;
                m.s_transform = `translateX(${(getIndex() - 1) * 100}%)`;
              }
            },
            children() {
              const div = fdom.div({
