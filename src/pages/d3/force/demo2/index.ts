...
export default function () {
  ...
  hookDestroy(
    subscribeEventListener(document, 'keydown', e => {
      if (e.key == 'Enter') {
        ...
        if (s) {
          ...
          links.set(
            links.get().filter(x => x.source != s.value && x.target != s.value)
          );
          didTick();
        }
      }
          ...
          fsvg.g({
            ...
            onClick(e) {
              ...
              if (e.shiftKey) {
                if (s) {
                  if (s != node.value) {
                    const oldIndex = links
                      .get()
                      .findIndex(
                        x => x.source == s.value && x.target == node.value.value
                      );
                    if (oldIndex < 0) {
                      links.set([
                        ...links.get(),
