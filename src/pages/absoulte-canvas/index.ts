import { createSignal, addEffect, arrayCountCreateWith, simpleFlex } from 'wy-helper'
import { 
  renderCanvas, 
  hookDraw, 
  hookDrawRect, 
...
export default function () {
  ...
  // 创建粒子
  function createParticle(id: number, containerWidth: number, containerHeight: number): Particle {
    return {
      id,
  ...
  renderCanvas({
    ...
    // 渲染粒子
    renderArrayKey(particles.get,v=>v.id,function(getParticle){
      hookDraw({
        ...
        draw(ctx) {
          ...
          const gradient = ctx.createRadialGradient(
            particle.size, particle.size, 0,
            particle.size, particle.size, particle.size * 3
          )
          ...
          // 粒子核心
          ctx.beginPath()
          ctx.arc(particle.size, particle.size, particle.size, 0, Math.PI * 2)
  ...
  hookDestroy(subscribeRequestAnimationFrame(function(){
