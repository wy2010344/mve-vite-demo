...
import { getSelectionRange, setSelectionRange,Range, preventDefault, contentEditable, browser, reDefine, debounce } from './mb'
...
function handleNewLine(
	...
	if(browser.type=="FF" || newLinePadding.length>0){
		preventDefault(e)
		insert('\n'+newLinePadding)
	}

	if(newLinePadding != padding && after[0] == "}"){
		const pos=getSelectionRange(editor)
		insert("\n"+padding)
		setSelectionRange(editor,pos)
	}
}
...
function deleteTab(editor:HTMLElement,start:number,padding:string,tab:string){
	...
	if(len>0){
		setSelectionRange(editor,{start,end:start+len}).deleteFromDocument()
	}
	return len
}
/**
...
function handleTabCharacters(editor:HTMLElement,tab:string,e:KeyboardEvent){
	...
	if(selected.length>0){
		...
		if(e.shiftKey){
			...
			//插入
			//第一行
			setSelectionRange(editor,{start,end:start})
			insert(tab)
			let nstart=before.length+inlines[0].length+tab.length+1
			//其它行
			let i=1
			while(i<inlines.length){
				setSelectionRange(editor,{start:nstart,end:nstart})
				insert(tab)
...
function handlePaste(
	editor:HTMLElement,
	hightlight:(e:HTMLElement,pos:Range)=>void,
	e:ClipboardEvent
){
	preventDefault(e)
...
export function codeJar(p:CodeJarOption){
	...
	const jar:CodeJar={
		...
		getSelection(){
			return editor?getSelectionRange(editor):selection
		},
		setSelection(v){
			setSelectionRange(editor,v)
		}
	}
	function orCallback(){
		if(p.callback){
		...
		vs.push(function(e:KeyboardEvent){
			...
			if(isUndo(e)){
				...
				if(record){
					...
					setSelectionRange(editor,record.pos)
				}
				if(at<0){at=0}
			}else
			if(isRedo(e)){
	...
	element.attr={
		contentEditable:reDefine(p.readonly,function(r){
			if(typeof(r)=='function'){
				return function(){
					return r()?undefined:contentEditable.text
				}
			}else{
				return r?undefined:contentEditable.text
			}
