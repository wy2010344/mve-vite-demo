...
function createRocks() {
  ...
  const rockCount = 20;
  const rockMaterial = hookDispose(
  ...
  for (let i = 0; i < rockCount; i++) {
    ...
    const rockMesh = hookRemoveFromParent(
      new THREE.Mesh(rockGeometry, rockMaterial)
    );
    parent.add(rockMesh);
...
function createTree() {
  ...
  const treeMaterial = hookDispose(
  ...
  for (let i = 0; i < treeCount; i++) {
    const treeMesh = hookRemoveFromParent(
      new THREE.Mesh(treeGeometry, treeMaterial)
    );
    parent.add(treeMesh);
...
function createBushes() {
  ...
  const bushMaterial = hookDispose(
    new THREE.MeshStandardMaterial({
      color: 0x80a040,
      flatShading: true,
    })
  ...
  for (let i = 0; i < bushCount; i++) {
    ...
    const bushMesh = hookRemoveFromParent(
